...
NEED_DEPLOY_PROJECT="springboot-helloword"
# Date/Time Veriables
LOG_DATE='date "+%Y-%m-%d"'
LOG_TIME='date "+%H:%M:%S"' 
CDATE=$(date "+%Y%m%d")
CTIME=$(date "+%H%M%S") 
#Shell Env
SHELL_NAME="deploy_d.sh"
SHELL_DIR="/deploy_d/log"
SHELL_LOG="${SHELL_DIR}/${SHELL_NAME}.log" 
#Code Env
JAR_DIR="/deploy_d/jar"
CONFIG_DIR="/deploy_d/config"
LOCK_FILE="/tmp/deploy_d.lock" 
# 定义变量
API_NAME="demo"
API_VERSION="0.0.1"
API_PORT=58080
...
#rm container
rm_con(){
    write_log "rm container..."
	cid=$(docker ps | grep "$CONTAINER_NAME" | awk '{print $1}')
	if [ "$cid" != "" ]; then
	   docker rm -f $cid
	fi
    write_log "rm container complete."	
}
...
#rm Dockerfile
rm_df(){
	write_log "rm Dockerfile..."
	rm -f Dockerfile
	write_log "rm Dockerfile complete."
}
